...
void Game::play()
{
	initBaseGame(screenWidth, ScreenHeight, "Puzzle Demo");
	engineLoop();
}
...
void Game::initGame(Renderer* renderer)
{
	...
	shapeA->setPosition(vec3(0.0f, 0.0f, 0.0f));
	...
	vector<int> tilemapLayout = { 0,  1,  1,   1,  2,
								 ...
								 16, 17, 17,  17, 18,
								 16, 17, 17,  17, 18,
								 64, 65, 65,  65, 66 }; //layout por indice
	...
	vector<vec2> tilemapLayout2 = { vec2(0,0) ,vec2(1,0) ,vec2(1,0) ,vec2(1,0) ,vec2(2,0)
								   ...
								   ,vec2(0,3) ,vec2(1,3) ,vec2(1,3) ,vec2(1,3) ,vec2(2,3) }; // layout por vectores
	...
	tileMap->setTileMap(5, 5, tilemapLayout, tilemapWalkable); // como parte de la inizializacion elijo que layout usar
